...
class KOTH_RoundEndModule: CF_ModuleWorld
{
    ...
    void AwardTopPerformers(KOTH_Settings ATPsettings)
    {
        ...
        if (mvp && mvp.Kills > 0)
        {
            ...
            if (mvpPlayer)
            {
                m_RewardManager.AddPlayerXP(mvpPlayer, ATPsettings.MVPBonusXP, "MVP Award");
                m_RewardManager.AddPlayerMoney(mvpPlayer, ATPsettings.MVPBonusMoney, "MVP Award");
                KOTH_NotificationModule.ShowNotificationAdvanced("MVP AWARD!", "$" + ATPsettings.MVPBonusMoney.ToString(), ARGB(255, 255, 215, 0), ATPsettings.MVPBonusXP.ToString() + "XP", ARGB(255, 255, 215, 0), ARGB(255, 218, 165, 32), 5.0, mvpPlayer.GetIdentity());
        ...
        if (sharpshooter && sharpshooter.Headshots > 0)
        {
            ...
            if (ssPlayer)
            {
                m_RewardManager.AddPlayerXP(ssPlayer, ATPsettings.SharpshooterBonusXP, "Sharpshooter Award");
                m_RewardManager.AddPlayerMoney(ssPlayer, ATPsettings.SharpshooterBonusMoney, "Sharpshooter Award");
                KOTH_NotificationModule.ShowNotificationAdvanced("SHARPSHOOTER AWARD!", "$" + ATPsettings.SharpshooterBonusMoney.ToString(), ARGB(255, 255, 140, 0), ATPsettings.SharpshooterBonusXP.ToString() + "XP", ARGB(255, 255, 140, 0), ARGB(255, 255, 69, 0), 5.0, ssPlayer.GetIdentity());
        ...
        if (medic && medic.Revives > 0)
        {
            ...
            if (medicPlayer)
            {
                m_RewardManager.AddPlayerXP(medicPlayer, ATPsettings.MedicBonusXP, "Top Medic Award");
                m_RewardManager.AddPlayerMoney(medicPlayer, ATPsettings.MedicBonusMoney, "Top Medic Award");
                KOTH_NotificationModule.ShowNotificationAdvanced("TOP MEDIC AWARD!", "$" + ATPsettings.MedicBonusMoney.ToString(), ARGB(255, 0, 255, 127), ATPsettings.MedicBonusXP.ToString() + "XP", ARGB(255, 0, 255, 127), ARGB(255, 60, 179, 113), 5.0, medicPlayer.GetIdentity());
                Print("[KOTH_RoundEndModule] Top Medic Award: " + medic.PlayerName + " (" + medic.Revives + " revives)");
            }
        }
    }
    
    void AwardTeamBonuses(string winningTeam, KOTH_RoundTeamStats eastStats, KOTH_RoundTeamStats westStats, KOTH_Settings ATBsettings)
    {
        array<Man> players = new array<Man>;
        GetGame().GetPlayers(players);
        ...
        for (int i = 0; i < players.Count(); i++)
        {
            ...
            if (team == winningTeam)
            {
                bonusXP = CalculateWinningBonus(playerStats, team, eastStats, westStats, ATBsettings, true);
                bonusMoney = CalculateWinningBonus(playerStats, team, eastStats, westStats, ATBsettings, false);
            ...
            {
                bonusXP = CalculateLosingBonus(playerStats, team, eastStats, westStats, ATBsettings, true);
                bonusMoney = CalculateLosingBonus(playerStats, team, eastStats, westStats, ATBsettings, false);
    ...
    int CalculateWinningBonus(KOTH_RoundPlayerStats playerStats, string team, KOTH_RoundTeamStats eastStats, KOTH_RoundTeamStats westStats, KOTH_Settings CWBsettings, bool isXP)
    {
        int totalTeamXP;
        int totalTeamMoney;
        ...
        int maxLevel = CWBsettings.MaxPlayerLevel;
        ...
        if (isXP)
        {
            bonus = totalTeamXP * levelRatio * CWBsettings.WinningTeamXPMultiplier * playTimeRatio;
        }
        else
        {
            bonus = totalTeamMoney * levelRatio * CWBsettings.WinningTeamMoneyMultiplier * playTimeRatio;
        }
        
        return bonus;
    }
    
    int CalculateLosingBonus(KOTH_RoundPlayerStats playerStats, string team, KOTH_RoundTeamStats eastStats, KOTH_RoundTeamStats westStats, KOTH_Settings CLBsettings, bool isXP)
    {
        int totalTeamXP;
        int totalTeamMoney;
        int teamFinalScore;
        ...
        int scoreLimit = CLBsettings.ScoreLimit;
        int playerLevel = playerStats.CurrentLevel;
        int maxLevel = CLBsettings.MaxPlayerLevel;
        float playTimeRatio = playerStats.PlayTimeRatio;
        ...
        if (isXP)
        {
            bonus = totalTeamXP * scoreRatio * levelRatio * CLBsettings.LosingTeamXPMultiplier * playTimeRatio;
        }
        else
        {
            bonus = totalTeamMoney * scoreRatio * levelRatio * CLBsettings.LosingTeamMoneyMultiplier * playTimeRatio;
        }
